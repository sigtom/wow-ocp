#!/bin/bash
# conductor/tests/verify_vlan_120.sh
...
echo "Starting VLAN 120 Verification..."

for NODE in $NODES; do
    echo "--- Checking $NODE ---"
    # Find IP in 172.16.120.x range
    IP=$(oc debug $NODE --quiet -- chroot /host ip -4 addr show | grep -oP '172\.16\.120\.\d+' | head -n 1)
    if [ -n "$IP" ]; then
        echo "SUCCESS: Found IP $IP on $NODE"
        NODE_IPS[$NODE]=$IP
    else
        echo "FAILURE: No 172.16.120.x IP found on $NODE"
        EXIT_CODE=1
    fi
done
