#!/bin/bash
# conductor/tests/verify_vlan_160.sh
...
echo "Starting VLAN 160 Verification..."

for NODE in $NODES; do
    echo "--- Checking $NODE ---"
    # Try to find IP on various interfaces (eno2 or eno2.160)
    IP=$(oc debug $NODE --quiet -- chroot /host ip -4 addr show | grep -oP '172\.16\.160\.\d+' | head -n 1)
    if [ -n "$IP" ]; then
        echo "SUCCESS: Found IP $IP on $NODE"
        NODE_IPS[$NODE]=$IP
    else
        echo "FAILURE: No 172.16.160.x IP found on $NODE"
        EXIT_CODE=1
    fi
done
