...
## Phase 1: Preparation (GitOps)
- [x] Task: Update `apps/plex/base/deployment.yaml` to add `PLEX_CLAIM` env var mapped to secret `plex-claim` key `claimToken`
- [x] Task: Create a placeholder `SealedSecret` for `plex-claim` (to satisfy Kustomize build)
- [x] Task: Update `apps/plex/base/kustomization.yaml` to include the secret
- [x] Task: Commit and Push preparation changes
