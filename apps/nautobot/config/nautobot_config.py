...
SECRET_KEY = os.getenv("NAUTOBOT_SECRET_KEY", "5=3pbtk3-x+3uin9y2&amp;qo8e&amp;oclg42-#z_y4@$odpav@_presh")
...
# (Deprecated) Storage backend to use for Job input files and Job output files.
#
...
SOCIAL_AUTH_KEYCLOAK_SECRET = 'nautobot-secret-2026'
SOCIAL_AUTH_KEYCLOAK_AUTHORIZATION_URL = 'https://keycloak.sigtom.dev/realms/pilot/protocol/openid-connect/auth'
SOCIAL_AUTH_KEYCLOAK_ACCESS_TOKEN_URL = 'https://keycloak.sigtom.dev/realms/pilot/protocol/openid-connect/token'
SOCIAL_AUTH_KEYCLOAK_PUBLIC_KEY = ''  # Use JWKS if available, otherwise simplified
# SOCIAL_AUTH_KEYCLOAK_ID_TOKEN_DECRYPTION_KEY = SOCIAL_AUTH_KEYCLOAK_SECRET # Only needed if ID token is encrypted (JWE)


def nautobot_is_superuser(response, user, backend, *args, **kwargs):
    if backend.name == 'keycloak':
        ...
        if is_admin:
            ...
            # Demote if not in group? Optional but safer
...
SOCIAL_AUTH_PIPELINE = (
    ...
    'nautobot_config.nautobot_is_superuser',
)
